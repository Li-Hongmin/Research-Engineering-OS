# Research Engineering OS

**Language**: [中文](../mdbook/) | [English](/)

![Book Cover](images/cover.png)

**Compress rework into standards + templates + checklists**

Author: Li Hongmin (李鸿敏)
The University of Tokyo, Computational Biology and Medical Sciences

---

## About This Book

This is not a book that teaches you “how to write code,” but a book that teaches you **how to manage research code**.

Target readers:
- AI/ML researchers
- Computational biology researchers
- AI for Science

Core ideas:
- **Experiments are the unit** (not code files)
- **Exploration can be messy, but outputs must be cleanable**
- **Conclusions can be temporarily fragile, but the chain of evidence must be solid**

---

## Read Online

This book is fully open source and can be read online for free. If you find it helpful, you’re welcome to:
- ⭐ Star it on [GitHub](https://github.com/your-username/research-engineering-os)
- 📖 Buy a [paperback/Kindle edition](https://kdp.amazon.com) to keep
- 💬 Share feedback and suggestions

---

## Version Information

- **Online version**: continuously updated, includes the latest content
- **Print edition**: v1.0, first published in February 2026

---

## Start Reading

Start from the [Preface](./00-preface.md), or jump directly to:
- [Why It Always Blows Up at the End](./01-why-flip.md) - understand the root cause
- [Experiments Are the Unit](./02-experiment-unit.md) - core concept
- [Repository Structure](./03-repo-layout.md) - practical guide

---

## Contact

- **Email**: lihongmin@edu.k.u-tokyo.ac.jp
- **Website**: [li-hongmin.github.io](https://li-hongmin.github.io)

---

© 2026 Li Hongmin. All rights reserved.