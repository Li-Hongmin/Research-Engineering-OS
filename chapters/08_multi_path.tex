...
\section{故事引入：从"灵活探索"到"不敢动任何东西"}

你的研究项目进行到第三个月。你是一个勤奋的研究者，尝试了很多不同的方向：

\begin{itemize}
  \item 路径 A：改进模型架构（5 种不同的注意力机制）
  \item 路径 B：优化训练策略（3 种不同的学习率调度）
  \item 路径 C：增强数据质量（4 种预处理方式）
  \item 路径 D：调整损失函数（6 种不同的损失组合）
\end{itemize}
...
\paragraph{问题 1：找不到最好的结果}

你记得某次实验效果很好，但你不记得是哪个配置、哪个输出目录。你开始一个一个打开目录，查看日志，试图找到那个结果。花了两个小时，还是不确定找对了没有。

\paragraph{问题 2：不敢删除任何东西}

outputs/ 已经占了 50GB 空间，但你不敢删除任何目录——万一删掉的正好是论文需要的那个实验呢？你决定"先留着，反正硬盘够大"。

\paragraph{问题 3：无法对比不同路径}

你想对比"路径 A（注意力改进）"和"路径 B（学习率优化）"的效果，但你发现：
\begin{itemize}
  \item 它们用的 baseline 不一样（一个是三个月前的，一个是最近的）
  \item 评估脚本不一样（一个算的是 top-1，一个是 top-5）
  \item 数据划分可能也不一样（记不清了）
\end{itemize}

\paragraph{问题 4：无法合并有效改进}

你在路径 A 里发现了一个有效的改进，想把它移植到路径 B 里，但发现：
\begin{itemize}
  \item 路径 A 和路径 B 的代码已经分化了
  \item 数据加载逻辑不兼容
  \item 合并需要大量人工工作
\end{itemize}

\textbf{你意识到：灵活探索变成了无序混乱，多路径并行变成了垃圾堆。}

\section{为什么多路径探索容易失控}

研究的本质就是\textbf{不确定性}：你不知道哪条路会成功，所以需要同时探索多个方向。但如果没有管理机制，探索的越多，混乱度就越高。

\subsection{失控的三个阶段}

\paragraph{阶段 1：快速探索（第 1-4 周）}

\textbf{行为：}
\begin{itemize}
  \item 想到什么就试什么，不拘泥于规范
  \item 代码复制粘贴，改个名字就用
  \item 输出随便找个地方放，"先跑起来再说"
\end{itemize}

\textbf{感觉：}充满活力，进展快速。

\paragraph{阶段 2：路径分化（第 5-8 周）}

\textbf{行为：}
\begin{itemize}
  \item 不同路径的代码开始分化，共同部分减少
  \item 每条路径有自己的数据处理、训练脚本、评估方式
  \item 新想法基于某个旧路径，而不是主线
\end{itemize}

\textbf{感觉：}有点乱，但还能记住大致情况。

\paragraph{阶段 3：失控混乱（第 9 周+）}

\textbf{行为：}
\begin{itemize}
  \item 完全记不住哪个实验是哪个路径的
  \item 不敢删除任何东西，空间占用暴增
  \item 想要合并改进时发现路径已经完全不兼容
  \item 准备论文时重新跑实验，结果对不上记忆
\end{itemize}

\textbf{感觉：}焦虑、无力、想推倒重来。

\subsection{根本原因：缺少"可丢弃"和"可合并"机制}

多路径探索的核心挑战是：
\begin{itemize}
  \item \textbf{不知道哪条路会成功}，所以需要并行探索多条路径
  \item \textbf{不能全部保留}，否则会淹没在信息海洋里
  \item \textbf{成功的路径需要合并回主线}，否则无法形成完整方案
\end{itemize}
...
\subsection{机制 1：每条路径必须隔离}

\textbf{隔离的三要素：}

\begin{enumerate}
  \item \textbf{独立的 Git 分支}
  \begin{verbatim}
  ...
  \begin{itemize}
    \item 每条路径的实验结果清晰分组
    \item 删除整个路径时，只需删除对应目录
    \item 归档时可以按路径打包
  \end{itemize}
\end{enumerate}

\subsection{机制 2：明确的生命周期管理}
...
\subsection{机制 3：公平对比的基准线}

所有路径对比时，必须使用\textbf{同一基准}：
...
\section{每周清理仪式：实验墓地整理}

\textbf{核心理念：}定期清理是避免垃圾堆的唯一方法。

\subsection{每周五下午的清理流程（30 分钟）}

\paragraph{Step 1：列出所有活跃路径（5 分钟）}
...
\subsection{Q1：路径太多了，记不住怎么办？}

\textbf{解决方案：}维护一个路径追踪表。
...
\subsection{Q2：不同路径的代码冲突怎么办？}

\textbf{预防胜于治疗：}
\begin{itemize}
  \item 路径尽量改不同的模块（如一个改数据、一个改模型）
  \item 共享的核心代码放在 src/，不轻易修改
  \item 路径特有的改动放在 experiments/ 里
\end{itemize}

\textbf{冲突发生时：}
\begin{itemize}
  \item 不要强行合并多个路径
  \item 先合并一个，验证成功后，再基于新 main 重新创建其他路径
  \item 或者：重新评估是否真的需要合并多个路径
\end{itemize}

\subsection{Q3：删除路径后后悔了怎么办？}
...
\begin{itemize}
  ...
  \end{verbatim}
  \item 删除前写简短总结（见上文"删除决策"）
  \item 重要的数据先归档到便宜的存储（如云端）
\end{itemize}
...
\section{10 分钟动作：整理当前的探索路径}

如果你现在只做一件事：清点并分类当前所有探索路径。

\begin{enumerate}
  \item \textbf{列出所有分支和输出}
  \begin{verbatim}
  git branch | grep "exp/"
  ls outputs/
  \end{verbatim}

  \item \textbf{为每条路径快速分类}
  ...
  创建 \texttt{docs/EXPLORATION\_TRACKER.md}，记录所有活跃路径。

  \item \textbf{设定下周清理时间}

  在日历里加入：\textbf{每周五 17:00 - 探索路径清理（30 分钟）}
\end{enumerate}

完成这个 10 分钟动作后，你会立刻感觉到：
\begin{itemize}
  \item 对项目状态的掌控力增强
  \item 知道哪些路径值得继续、哪些该放弃
  \item 不再担心"垃圾堆"会失控
\end{itemize}

\textbf{记住：多路径探索是研究的必要特征，但不管理的多路径探索会变成灾难。定期清理不是负担，而是保持清醒的必要仪式。}
