...
\begin{itemize}
  \item 审查所有 AI 生成的代码，找出隐藏问题；
  \item 统一不一致的实现；
  \item 简化过度设计的架构；
  \item 补充缺失的测试和验证。
\end{itemize}

你意识到：\textbf{AI 编码助手不是问题，问题是你的工作流没有跟上。}

在 AI 加入之前，你的代码量少，错误少，可以靠记忆和经验把控质量。但现在代码量暴增，而你的验证机制还停留在"手工审查"阶段，这个缺口会越来越大。

\section{AI 编码的三大陷阱}

\subsection{陷阱 1：生成速度超过验证能力}

\textbf{症状：}AI 一分钟生成 100 行代码，你觉得"看起来没问题"就接受了。但你其实没有仔细检查边界条件、错误处理、性能影响。

\textbf{后果：}
\begin{itemize}
  \item 埋下大量隐藏 bug，在关键时刻暴露；
  \item 代码库充斥着"能跑但不健壮"的实现；
  \item 重构成本远超一开始就做对的成本。
\end{itemize}

\textbf{本质问题：}验证机制没有升级，还停留在"手工审查"阶段。

\subsection{陷阱 2：重复生成导致不一致}

\textbf{症状：}你需要类似功能时，让 AI 重新生成，而不是复用现有代码。结果项目里有多个"类似但不完全相同"的实现。

\textbf{后果：}
\begin{itemize}
  \item 修改一个逻辑需要改多处，容易遗漏；
  \item 不同实现可能有微妙差异，导致结果不一致；
  \item 代码库膨胀，维护成本暴增。
\end{itemize}

\textbf{本质问题：}没有区分"核心库代码"和"一次性胶水代码"（回顾第3章）。

\subsection{陷阱 3：过度依赖"顺手改一下"}

\textbf{症状：}在让 AI 实现一个功能时，你顺便让它"把这个也改了"、"把那个也优化了"。一个 commit 包含了十几个文件的改动。

\textbf{后果：}
\begin{itemize}
  \item 无法定位哪个改动引起了问题；
  \item 想要回滚时发现"牵一发动全身"；
  \item 代码历史混乱，无法追溯逻辑演进。
\end{itemize}

\textbf{本质问题：}改动不再是"可验证的最小单元"（回顾第1章）。

\section{升级后的工作流：把 AI 当成"快速试错的初级程序员"}

\textbf{核心理念：}AI 是助手，不是替代品。它的优势是快速生成初稿，你的责任是验证、整合、把关。

把 AI 编码看作两阶段过程：
\begin{enumerate}
  \item \textbf{生成阶段：}AI 快速产出初稿（快速、粗糙、允许有问题）；
  \item \textbf{验证阶段：}人类审查、测试、重构（慢速、严格、确保质量）。
\end{enumerate}

\textbf{关键原则：}生成可以快，但上线必须慢。

\subsection{原则 1：每次只做一个可验证的改动}

\textbf{操作建议：}
\begin{itemize}
  \item 每次让 AI 生成的代码不超过 200 行 diff；
  \item 每次改动只涉及一个逻辑功能（如"添加数据增强"、"修复 NaN bug"）；
  \item 改动必须能独立验证（有对应的测试或实验）。
\end{itemize}
...
\textbf{验证清单（每次改动后检查）：}
\begin{itemize}
  \item[$\square$] 代码能运行（不崩溃）
  \item[$\square$] 输出符合预期（shape、数值范围、数据类型）
  \item[$\square$] 边界情况处理正确（空输入、极端值、错误输入）
  \item[$\square$] 不影响现有功能（回归测试通过）
  \item[$\square$] 有测试覆盖（至少 smoke test）
\end{itemize}

\subsection{原则 3：禁止"顺手重构一大片"}

\textbf{症状识别：}

危险信号：
\begin{itemize}
  \item AI 提议："我顺便帮你重构一下这部分代码，让它更优雅"
  \item 你想："反正都要改，不如一起做了"
  \item 结果：一个 PR 包含功能添加 + 重构 + bug 修复
\end{itemize}

\textbf{正确做法：}
\begin{enumerate}
  \item \textbf{先实现功能，后重构：}
  \begin{verbatim}
  # Step 1: 实现功能（允许粗糙）
  ...
  \item \textbf{重构与功能分离：}
  \begin{itemize}
    \item 功能改动：commit message 用 "feat:" 或 "fix:"
    \item 重构改动：commit message 用 "refactor:"
    \item 绝不混在一起
  \end{itemize}
\end{enumerate}

\subsection{原则 4：核心逻辑必须人工审查}

\textbf{定义"核心逻辑"：}

以下代码不能盲目接受 AI 生成的版本，必须人工仔细审查：
\begin{itemize}
  \item \textbf{数据处理：}数据加载、预处理、划分、增强
  \item \textbf{模型核心：}损失函数、关键模块（注意力、归一化等）
  \item \textbf{评估逻辑：}指标计算、后处理、阈值选择
  \item \textbf{随机性控制：}随机种子设置、随机采样逻辑
  \item \textbf{超参数：}学习率调度、权重衰减、dropout 率
\end{itemize}

\textbf{审查清单：}
\begin{itemize}
  \item[$\square$] 逻辑正确性（算法实现对吗？）
  \item[$\square$] 边界条件（空数据、极端值会怎样？）
  \item[$\square$] 数据一致性（训练和测试逻辑一致吗？）
  \item[$\square$] 随机性（种子设置正确吗？可复现吗？）
  \item[$\square$] 性能影响（会不会特别慢？内存会爆吗？）
\end{itemize}

\subsection{原则 5：先补最小测试，再改核心逻辑}
...
\textbf{测试先行的好处：}
\begin{itemize}
  \item 明确当前行为（作为回归测试基准）
  \item 快速验证重构（1 分钟而不是 1 小时实验）
  \item 避免破坏现有功能
  \item 强制思考"正确的行为是什么"
\end{itemize}

\section{AI 规则页（CLAUDE.md）：团队协作的共识}

为了在团队中统一 AI 使用规范，建议在项目根目录创建 \texttt{CLAUDE.md}（或 \texttt{AI\_RULES.md}），明确 AI 编码的边界和流程。

\subsection{CLAUDE.md 模板}
...
\section{实战案例：AI 辅助的正确打开方式}

\subsection{案例 1：添加新模型组件}
...
\subsection{Q1：AI 生成的代码太复杂，看不懂怎么办？}

\textbf{解决方案：}
\begin{itemize}
  \item 拒绝接受！让 AI 重新生成更简单的版本。
  \item 提示词：\begin{verbatim}
  请实现 XXX 功能，要求：
  - 代码简单直接，避免过度抽象
  - 尽量使用标准库和常见模式
  - 添加详细注释解释每个步骤
  \end{verbatim}
  \item 记住：\textbf{你看不懂的代码，未来的你也看不懂，别人更看不懂。}
\end{itemize}

\subsection{Q2：AI 生成的代码有 bug，但改了又出现新 bug}

\textbf{解决方案：}
\begin{itemize}
  \item 不要反复让 AI 修 bug，陷入无限循环。
  \item 正确做法：
  \begin{enumerate}
    \item 先写测试，复现 bug
    \item 人工修复 bug（小范围改动）
    \item 测试通过后，再考虑是否需要重构
  \end{enumerate}
  \item AI 可以提示可能的问题，但修复应该由人来做。
\end{itemize}

\subsection{Q3：团队成员不遵守 AI 使用规范怎么办？}

\textbf{解决方案：}
\begin{itemize}
  \item 在 Git pre-commit hook 里加入自动检查：
  \begin{verbatim}
  ...
  \item 在 CI 中强制执行（见下节）
  \item Code review 时严格检查
\end{itemize}

\section{10 分钟动作：为下一次 AI 编码建立检查点}

如果你现在只做一件事：为下一次 AI 辅助编码建立最小验证流程。

\begin{enumerate}
  \item \textbf{创建 CLAUDE.md}

  复制本章的模板，放到项目根目录。

  \item \textbf{写一个 smoke test}
  \begin{verbatim}
  # tests/test_smoke.py
...
从下一次使用 AI 编码开始，\textbf{默认行为}就是：生成 → 审查 → 测试 → 验证 → 合并。这个流程会成为肌肉记忆，确保 AI 始终是你的提速器，而不是地雷制造机。
