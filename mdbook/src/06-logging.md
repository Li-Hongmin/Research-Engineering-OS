# 实验记录自动化：缺的不是工具，是默认行为 {#ch:logging**
...
代码里写的数据路径是 `data/v2/`，但你现在的数据目录是 `data/v3/`。你不记得当时是不是换过数据版本。你在聊天记录里搜索\"数据\"，试图找到线索。
...
问题不是缺少工具（MLflow、W&B、TensorBoard 都很好），而是**缺少记录的默认行为**：很多人觉得\"这次只是试试，不用记录\"，结果试着试着就忘了记录，最后有价值的实验也没留下痕迹。

## 两层日志策略：机器精确 + 人类简洁

实验记录的核心挑战是平衡两个需求：

- **机器需要完整精确的信息**（用于复现和自动化分析）；

- **人类需要简洁可读的摘要**（用于快速回顾和决策）。
...
- **自动生成：**不依赖人工输入，通过脚本自动收集；

- **结构化：**JSON 格式，便于程序解析和查询；
...
- **run_id：**唯一标识，建议用时间戳+简短描述（见第2章）。

- **git.commit：**代码版本，用 `git rev-parse HEAD` 获取。

- **git.dirty：**是否有未提交的修改，用 `git diff-index --quiet HEAD --` 检查。如果为 true，建议保存 diff：`git diff > changes.patch`。

- **config.resolved：**展开后的最终配置，包含所有默认值。这很重要，因为默认值可能随代码更新而变化。

- **data.hash：**数据版本的哈希值，确保数据完全一致。可以用 `sha256sum` 计算整个数据目录的哈希，或使用 DVC 等工具。

- **environment.pip_freeze_hash：**依赖版本的哈希，用 `pip freeze | sha256sum` 计算。避免完整保存 pip freeze 输出（太长），只保存哈希和原始文件路径。
...
- **降低记录门槛：**如果要写一篇长文档，你会拖延；5 行的话，2 分钟就能完成。

- **强制提炼核心：**逼迫你思考\"这次实验到底验证了什么\"，而不是流水账。

- **快速回顾：**几个月后再看，5 行摘要比完整日志更有用。

## 自动化工具：让记录成为零成本行为

**核心理念：**记录不应该依赖\"记得去做\"，而应该自动发生。
...
**重点：**工具是辅助，核心是**记录的规范**。即使没有 MLflow/W&B，有 run.json 和 run.md 也足够。
...
**问题：**如果保存完整的 config（包含模型定义、数据预处理细节等），run.json 可能很大。

**解决方案：**

- 只保存\"关键超参数\"在 run.json 里（如学习率、batch size）；

- 完整 config 保存到单独文件：`config_resolved.yaml`；

- run.json 里只记录 config 文件路径和哈希。
...
- 使用数据版本管理工具（如 DVC、Git LFS）；

- 或者只记录数据的\"清单文件\"（manifest）：
...
- 在 run.json 里记录清单文件路径和哈希。

## 10 分钟动作：为下一个实验建立自动记录

如果你现在只做一件事：建立自动记录的最小系统。

1.  **复制 run_logger.py**

    把前面的 `log_run` 函数复制到你的项目里。
...
- outputs/\<run_id\>/run.json 自动生成了

- outputs/\<run_id\>/run.md 模板生成了

- run.json 里的信息完整（git、config、env、metrics）
