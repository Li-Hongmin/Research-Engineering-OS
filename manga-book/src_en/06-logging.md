# Chapter 6: The Art of Record-Keeping

**Log Archaeology**

### Cell 1

![三个月后，小研试图找回实验细节](../../images/06-logging/06_001.png)

*Three months later, Rina tried to recall the experimental details*

---

### Cell 2

![翻找笔记：只有零散片段](../../images/06-logging/06_002.png)

*Searching through notes: only scattered fragments*

---

### Cell 3

![找配置：到底用的哪个？](../../images/06-logging/06_003.png)

*Find the configuration: which one is actually being used?*

---

### Panel 4

![领悟：当时的我对未来的我太残忍](../../images/06-logging/06_004.png)

*Insight: Back then, I was too cruel to my future self*

---

### Panel 5

![解决方案：双层日志系统](../../images/06-logging/06_005.png)

*Solution: Dual-layer Logging System*

---

### Panel 6

![第一层：自动记录（JSON）](../../images/06-logging/06_006.png)

*Layer 1: Automatic Logging (JSON)*

---

### Panel 7

![第二层：人工笔记（Markdown）](../../images/06-logging/06_007.png)

*Layer 2: Handwritten Notes (Markdown)*

---

### Panel 8

![run.json结构：机器的完美记忆](../../images/06-logging/06_008.png)

*Structure of run.json: the machine’s perfect memory*

---

### Panel 9

![run.md模板：人类的快速记录](../../images/06-logging/06_009.png)

*run.md Template: Rapid Notes for Humans*

---

### Cell 10

![工作流：实验前中后的记录](../../images/06-logging/06_010.png)

*Workflow: Records Before, During, and After the Experiment*

---

### Grid 11

![好处1：三个月后立即找到](../../images/06-logging/06_011.png)

*Benefit 1: Find one immediately after three months*

---

### The 12th Cell

![好处2：审稿人要数据秒回](../../images/06-logging/06_012.png)

*Benefit 2: Reviewers want data—respond instantly*

---

### Panel 13

![好处3：团队成员能接手](../../images/06-logging/06_013.png)

*Benefit 3: Team members can take over*

---

### Grid 14

![好处4：写论文素材齐全](../../images/06-logging/06_014.png)

*Benefit 4: Comprehensive materials for writing papers*

---

### Square 15

![工具：MLflow/W&B集成](../../images/06-logging/06_015.png)

*Tools: MLflow/W&B integration*

---

### Panel 16

![工具：自动化脚本](../../images/06-logging/06_016.png)

*Tool: Automation Script*

---

### Panel 17

![实践：5分钟日志习惯](../../images/06-logging/06_017.png)

*Practice: The 5-Minute Journaling Habit*

---

### Grid 18

![技巧：记录失败和惊喜](../../images/06-logging/06_018.png)

*Tip: Record failures and surprises*

---

### Panel 19

![10分钟行动：为下次实验设置日志](../../images/06-logging/06_019.png)

*10-Minute Action: Set Up Logging for the Next Experiment*

---

### Cell 20

![下一章预告：AI助手的陷阱](../../images/06-logging/06_020.png)

*Next Chapter Preview: The Trap of the AI Assistant*

---